...
class SentimentExampleBatchIterator:
    ...
    def get_next_batch(self):
        if self._cur_idx < len(self.data): # loop over the dataset
            ...
            batch_inputs = []
            batch_lengths = []
            for batch_data in batch_exs:
                batch_labels.append(batch_data.label)
                batch_lengths.append(len(batch_data.words))
            max_size_padding = max(batch_lengths)
            for batch_data in batch_exs:
                batch_inputs.append( np.pad(batch_data.word_indices, (0,  max_size_padding - len(batch_data.word_indices))))  
